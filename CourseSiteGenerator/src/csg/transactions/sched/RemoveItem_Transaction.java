...
public class RemoveItem_Transaction implements jTPS_Transaction{
    ...
    public RemoveItem_Transaction(CSGApp app, ScheduleItem item){
        this.app = app;
        this.item = item;
    }
    ...
    public void undoTransaction(){
        ...
        SchedData schedData = app.getCSGData().getSchedData();
        if(item != null){
            schedData.addItem(item.getType(), item.getDate(), item.getTime(), item.getTitle(), item.getTopic(), item.getLink(), item.getCriteria());
        }
    }
